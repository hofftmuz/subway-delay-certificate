 ...
 * @throws ValidationError - 검증 실패 시
 */
function validateInqrDate(inqrDate) {
    // 기본값 처리 및 빈 문자열 체크
    ...
    if (!inqrDate || inqrDate === '') {
        ...
        return (0, date_fns_1.format)((0, date_fns_1.startOfDay)(new Date()), 'yyyyMMdd');
    }
    ...
    let date;
    try {
        date = (0, date_fns_1.parse)(inqrDate, 'yyyyMMdd', new Date());
    }
    catch (error) {
        throw new types_1.ValidationError(error_codes_1.ERROR_CODES.INVALID_DATE_FORMAT.code, error_codes_1.ERROR_CODES.INVALID_DATE_FORMAT.msg);
    }
    ...
    try {
        const formatted = (0, date_fns_1.format)(date, 'yyyyMMdd');
    ...
    if ((0, date_fns_1.isFuture)((0, date_fns_1.startOfDay)(date))) {
        throw new types_1.ValidationError(error_codes_1.ERROR_CODES.FUTURE_DATE.code, error_codes_1.ERROR_CODES.FUTURE_DATE.msg);
    }
    return inqrDate;
}
/**
...
function validateDelayTime(delayTime) {
    ...
    if (!delayTime || delayTime === '') {
        if (delayTime === '') {
            throw new types_1.ValidationError(error_codes_1.ERROR_CODES.INVALID_INPUT.code, error_codes_1.ERROR_CODES.INVALID_INPUT.msg);
        }
        return '30';
    }
    // 숫자 형식 검증
 ...
 * @throws ValidationError - 검증 실패 시
 */
function validatePdfDataYn(pdfDataYn) {
    // 기본값 처리 및 빈 문자열 체크
