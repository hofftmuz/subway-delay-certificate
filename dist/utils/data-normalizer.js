...
const time_range_mapper_1 = require("./time-range-mapper");
 ...
 */
function normalizeData(data, date) {
    console.log(`[DataNormalizer] 정규화 시작: ${data.length}건`);
    const normalized = [];
    for (const item of data) {
        try {
            // 1. 시간대 매핑을 통한 delayStart, delayEnd 계산
            const { delayStart, delayEnd } = (0, time_range_mapper_1.mapTimeRange)(item.timeRange, item.site, date);
            // 2. delayDate 생성 (YYMMDD 형식)
            const baseDate = (0, date_fns_1.parse)(date, 'yyyyMMdd', new Date());
            const delayDate = (0, date_fns_1.format)(baseDate, 'yyMMdd');
