...
exports.mapTimeRange = mapTimeRange;
const date_fns_1 = require("date-fns");
const time_range_mappings_1 = require("../config/time-range-mappings");
/**
 * 시간대 매핑
 *
 * 사이트별 시간대 문자열을 실제 시간 범위로 변환
 ...
 */
function mapTimeRange(timeRange, site, date) {
    // 1. site에 따라 매핑 선택
    const timeRanges = site === 'seoulmetro'
        ? time_range_mappings_1.SEOUL_METRO_TIME_RANGES
        : time_range_mappings_1.KORAIL_TIME_RANGES;
    // 2. timeRange로 매핑 조회
    ...
    // 3. 기준 날짜 파싱
    const baseDate = (0, date_fns_1.parse)(date, 'yyyyMMdd', new Date());
    // 4. delayStart 계산 (같은날 + startHour)
    const startDate = new Date(baseDate);
    startDate.setHours(mapping.startHour, 0, 0, 0);
    const delayStart = (0, date_fns_1.format)(startDate, 'yyyyMMddHHmm');
    // 5. delayEnd 계산
    let endDate;
    if (mapping.nextDay) {
        // 막차 시간대: 다음날 + endHour
        endDate = (0, date_fns_1.addDays)(baseDate, 1);
    ...
    const delayEnd = (0, date_fns_1.format)(endDate, 'yyyyMMddHHmm');
    return {
        delayStart,
        delayEnd
    };
}
//# sourceMappingURL=time-range-mapper.js.map