...
function handleAxiosError(error, context) {
    ...
    if (error.code === 'ECONNREFUSED') {
        ...
        throw new Error(error_types_1.ERROR_TYPES.CONNECTION_REFUSED);
    }
    // 서버 에러 처리 (5xx)
    if (error.response?.status && error.response.status >= 500) {
        console.error(`${contextPrefix} 서버 에러 발생:`, error.response.status, error.response.statusText);
        throw new Error(error_types_1.ERROR_TYPES.SERVER_ERROR);
    }
    // 기타 Axios 네트워크 에러
