...
import { USER_AGENT } from '../config/http-config';
...
export async function downloadMultiplePdfs(
  pdfUrls: Array<{ url: string; site: 'seoulmetro' | 'korail' }>
): Promise<string[]> {
  console.log(`[PdfDownloader] 병렬 PDF 다운로드 시작: ${pdfUrls.length}건`);

  // 병렬로 다운로드 (각각 독립적인 브라우저 인스턴스 사용)
  const promises = pdfUrls.map(({ url, site }) =>
    downloadPdfAsBase64(url, site).catch(error => {
