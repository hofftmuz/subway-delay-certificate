...
import { InputDTO, ValidatedInput, ScrapedDelayInfo, FormattedDelayData, OutputDTO, ValidationError } from '../types';
import { validateInput } from '../utils/input-validator';
import { SeoulMetroScraper } from '../scrapers/seoul-metro-scraper';
import { KorailScraper } from '../scrapers/korail-scraper';
import { normalizeData } from '../utils/data-normalizer';
import { createSuccessResponse, createErrorResponse } from '../utils/response-formatters';
import { ERROR_CODES } from '../config/error-codes';
import { ERROR_TYPES } from '../config/error-types';
import { downloadMultiplePdfs } from '../utils/pdf-downloader';
 ...
 */
export class DelayScraperService {
  private seoulMetroScraper: SeoulMetroScraper;
  private korailScraper: KorailScraper;

  constructor() {
    this.seoulMetroScraper = new SeoulMetroScraper();
    this.korailScraper = new KorailScraper();
  }
   ...
   * @returns API 출력 데이터
   */
  async processRequest(input: InputDTO): Promise<OutputDTO> {
    // 1. 입력 검증
   ...
   */
  async scrape(date: string): Promise<ScrapeResult> {
    ...
    let korailPromise: Promise<ScrapedDelayInfo[]>;
    ...
    const allData: ScrapedDelayInfo[] = [];
    ...
    if (results[0].status === 'fulfilled') {
      ...
      if (error instanceof Error) {
        ...
        if (error.message === ERROR_TYPES.NETWORK_ERROR ||
            error.message === ERROR_TYPES.SERVER_ERROR ||
            error.message === ERROR_TYPES.CONNECTION_REFUSED) {
  ...
  normalize(data: ScrapedDelayInfo[], date: string): FormattedDelayData[] {
    return normalizeData(data, date);
  }
  ...
  formatResponse(
    ...
    return createSuccessResponse(data, hasPartialFailure);
  }
  ...
  private getDaysAgo(date: string): number {
    ...
    return differenceInDays(today, startOfDay(inputDate));
  }
   ...
   */
  private shouldScrapeKorail(date: string): boolean {
    const daysAgo = this.getDaysAgo(date);
    return daysAgo <= 7;
  }
  ...
  private async downloadPdfs(
    formattedData: FormattedDelayData[],
    scrapedData: ScrapedDelayInfo[]
  ): Promise<void> {
    ...
    // PDF URL이 있는 항목만 필터링
    const pdfItems: Array<{ index: number; url: string; site: 'seoulmetro' | 'korail' }> = [];
    ...
    // 병렬로 PDF 다운로드
    const pdfResults = await downloadMultiplePdfs(
      pdfItems.map(({ url, site }) => ({ url, site }))
    );
    ...
    pdfItems.forEach(({ index }, i) => {
      ...
      if (pdfBase64 && pdfBase64.trim() !== '') {
        ...
        console.warn(
          `[Service] PDF 다운로드 실패: ${formattedData[index].line} ${formattedData[index].direction} ${formattedData[index].timeRange}`
        );
      }
    });
    ...
    console.log(
      `[Service] PDF 다운로드 완료: ${successCount}/${pdfItems.length}건 성공${failureCount > 0 ? `, ${failureCount}건 실패` : ''}`
    );
  }
}
